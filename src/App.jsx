...
import { addAwards, addHistory, addStamps, getAwards, getHistory, getMistakes, getSettings, getStamps, saveSettings } from "./utils/storage";
...
export default function App() {
  ...
  const finishQuiz = (summary) => {
    ...
    setResult({
      ...summary,
      conquered,
      masteredCount: session?.weakMode ? summary.correct : summary.masteredCount || 0
    });
    setNewStamps(earned);
  ...
  const route = useMemo(() => {
    ...
    return null;
  }, [view, history, mistakes, stamps, awards, session, quizQuestions, result, newStamps, newAwards, settings]);
